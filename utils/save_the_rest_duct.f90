subroutine save_the_rest_duct(fname,geometry,ntt,t_hist,means,vars,skews,kurts,medians,nby,nbz,&
...
implicit none
     ...
     double precision, dimension(1:ntt), intent(in)                        :: t_hist,means,vars,skews,kurts,medians
     ...
     character(len=1024)                                                   :: channel,duct,ellipse,triangle,racetrack
     channel = "channel"
     duct = "duct"
     ellipse = "ellipse"
     triangle = "triangle"
     ...
     dsetname = "geometry"
     descr = "Problem geometry: Channel=0, Duct=1, Ellipse=2, Triangle=3, Racetrack=4, Other=5"
     ...
     else if (geometry .eq. triangle) then
          call hdf_add_1d_darray_to_file(1,3,fname,dsetname,descr)
     else if (geometry .eq. racetrack) then
          call hdf_add_1d_darray_to_file(1,4,fname,dsetname,descr)
     else
          call hdf_add_1d_darray_to_file(1,5,fname,dsetname,descr)
     end if
